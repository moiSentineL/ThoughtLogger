...
logging.basicConfig(filename='logging.log', level=logging.ERROR, format="%(asctime)s: %(exc_type)s %(message)s in Line No. %(errorline)s")
...
def setcustomattr(*args, **kwargs):
    ...
    result = strexc.split("'")[1]

    record = old_factory(*args, **kwargs)
    record.exc_type = result
